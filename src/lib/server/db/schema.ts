...
export const contractsTable = pgTable('contratos', {
	...
	esfera: varchar("esfera", { length: 13 }).notNull(),
	poder: varchar("poder", { length: 13 }).notNull()
},
	(table) => [
		check('esfera_valida', sql`${table.esfera} IN ('Federal', 'Estadual', 'Municipal', 'Não se aplica')`),
		check('poder_valido', sql`${table.poder} IN ('Executivo', 'Legislativo', 'Judiciário', 'Não se aplica')`)
	]
);
