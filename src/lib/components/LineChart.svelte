<script lang="ts">
    ...
    onMount(() =>
    {
        ...
        const chartConfig: ChartConfiguration<'line'> = {
            ...
            options: {
                    ...
                    y: {
                        ...
                        ticks: {
                            ...
                            callback: function(value)
                            {
                                ...
                                {
                                    const formatted = BRLCurrencyFormatter
                                                        .formatToParts(value)
                                                        .slice(0, 3)
                                                        .map(({type, value}) => value)
                                                        .join('');

                                    return value === 0 ? formatted : formatted + " M";
                                }
                ...
                plugins: {
                    title: {
                        display: true,
                        text: "DESPESAS DIÁRIAS EM CONTRATOS VIGENTES",
                        color: systemColors.text
                    },
                    ...
                    tooltip: {
                        callbacks: {
                            ...
                            label: function(context) {
                                ...
                                if (context.parsed.y) {
                                    label += BRLCurrencyFormatter.format(context.parsed.y);
                                }
                                else
                                {
                                    label = "N/A";
        ...
        const handler = function(e: MediaQueryListEvent)
        {
            ...
            const accentColor = e.matches ? 'hsl(37, 100%, 64%)': 'hsl(37, 100%, 34%)';
